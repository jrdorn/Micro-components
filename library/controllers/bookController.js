...
exports.book_update_get = function (req, res, next) {
  ...
  async.parallel(
    ...
    function (err, results) {
      ...
      // success- mark the selected genres as checked
      for (
        let all_genres = 0;
        all_genres < results.genres.length;
        all_genres++
      ) {
        for (
          let book_genre = 0;
          book_genre < results.book.genre.length;
          book_genre++
        ) {
          if (
            results.genres[all_genres]._id.toString() ===
            results.book.genre[book_genre]._id.toString()
          ) {
...
exports.book_update_post = [
  ...
  (req, res, next) => {
    ...
    let book = new Book({
      ...
      _id: req.params.id, // _id required so a new ID will not be assigned
    });
    ...
    if (!errors.isEmpty()) {
      ...
      Book.findByIdAndUpdate(
        ...
        book,
        {},
        function (err, curr_book) {
          if (err) {
            return next(err);
          }
          // success- redirect to book detail page
          res.redirect(curr_book.url);
        }
      );
    }
  },
